...
def get_shelters(db: Session = Depends(get_db)):
    ...
    for shelter in shelters:
        shelter.safe_capacity = ShelterService.calculate_safe_capacity(shelter)
...
def get_shelter_capacity(shelter_id: int, db: Session = Depends(get_db)):
...
def allocate_evacuees_endpoint(
    zone_id: int,
    evacuee_count: int,
    ...
    result = ShelterService.allocate_evacuees(db, zone_id, evacuee_count)
    
    return result

@router.get("/alerts")
def get_capacity_alerts(db: Session = Depends(get_db)):
    """Get shelters approaching capacity"""
    
    alerts = ShelterService.check_capacity_overflow(db)
